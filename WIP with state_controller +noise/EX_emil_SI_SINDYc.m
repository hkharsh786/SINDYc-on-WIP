clear all, close all 
clc
figpath = './FIGURES/WIP_noise/';mkdir(figpath)
datapath = './DATA/WIP_noise/'; mkdir(datapath)
...
%% Generate Data 
InputSignalType = 'type4ct'; %'torque'; %type4ct'; %type4; type4a; noise; 
...
polyorder =1 ;
usesine = 1;
lambda=99; %works good for gain 500, 1 0.1, fot ct , also 10 20 20 20 gives  better result
%lambda_vec = [10,20,20,20]; %can be defined as vec tor as per no of states
% eps = 0;  

%%Add noise
%noise have been updated to state wise value .
eps  = 0.05*std(x);
x = x + eps.*randn(size(x));
filter_order = 11;
filter_framelen = 13;
% filter_smoothingfactor=0.1;
